...
void LCD_SetPixel(uint16_t Xpos,uint16_t Ypos, uint16_t point)
{
	uint16_t increment = 1;
	
	if( (Xpos + increment >= MAX_X || Ypos + increment >= MAX_Y) || ((Xpos - increment < 0) || (Ypos - increment < 0)))
	{
		return;
	}
	LCD_SetPoint(Xpos - 1, Ypos - 1, point);
	LCD_SetPoint(Xpos, Ypos - 1, point);
	LCD_SetPoint(Xpos + 1, Ypos - 1, point);
	LCD_SetPoint(Xpos - 1, Ypos, point);
...
void LCD_DrawBiggerLine( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint16_t color ){
    ...
    if( dx == 0 )     /* X轴上没有增量 画垂直线 */ 
    {
        do
        { 
            LCD_SetPixel(x0, y0, color);   /* 逐点显示 描垂直线 */
            y0++;
        }
        while( y1 >= y0 ); 
		return; 
    ...
    if( dy == 0 )     /* Y轴上没有增量 画水平直线 */ 
    {
        do
        {
            LCD_SetPixel(x0, y0, color);   /* 逐点显示 描水平线 */
            x0++;
        }
        while( x1 >= x0 ); 
		return;
...
void myPixel(uint16_t xc,uint16_t yc,uint16_t x,uint16_t y, uint16_t color, int flag)
{
	if(flag == RIGHT){
...
void myPixel2(uint16_t xc,uint16_t yc,uint16_t x,uint16_t y, uint16_t color)
{
		LCD_SetPoint(xc-x,yc-y,color);
		LCD_SetPoint(xc-y,yc-x,color);
}
void heart(uint16_t Xpos,uint16_t Ypos,uint16_t color){
	LCD_SetPixel(Xpos, Ypos, color);
	LCD_SetPixel(Xpos + 2, Ypos, color);
	LCD_SetPixel(Xpos + 6, Ypos, color);
	LCD_SetPixel(Xpos + 8, Ypos, color);
	
	LCD_DrawBiggerLine(Xpos - 2, Ypos + 2, Xpos + 10, Ypos + 2, color);
	LCD_DrawBiggerLine(Xpos - 2, Ypos + 4, Xpos + 10, Ypos + 4, color);
	LCD_DrawBiggerLine(Xpos, Ypos + 6, Xpos + 8, Ypos + 6, color);
	
	LCD_SetPixel(Xpos + 2, Ypos + 8, color);
	LCD_SetPixel(Xpos + 4, Ypos + 8, color);
	LCD_SetPixel(Xpos + 6, Ypos + 8, color);
	
	LCD_SetPixel(Xpos + 4, Ypos + 10, color);
}

void LCD_DrawHalfCircle2(uint16_t Xpos,uint16_t Ypos, uint16_t radius, uint16_t color)
{

	int x=0;
	int y=radius;
	int p=1-radius;
	myPixel2(Xpos,Ypos,x,y, color);
...
void LCD_DrawHalfCircle(uint16_t Xpos,uint16_t Ypos, uint16_t radius, uint16_t color, int flag)
{

	int x=0;
	int y=radius;
	int p=1-radius;
	myPixel(Xpos,Ypos,x,y, color, flag);
...
void LCD_DrawFilledCircle(uint16_t Xpos,uint16_t Ypos, uint16_t radius, uint16_t color)
{
	while (radius > 0){
	int x=0;
	int y=radius;
...
void LCD_DrawThickRectangle (uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t color)
{
	
	LCD_DrawBiggerLine(X_start, Y_start, X_start + width, Y_start, color);
	LCD_DrawBiggerLine(X_start, Y_start, X_start, Y_start + height, color);
	LCD_DrawBiggerLine(X_start, Y_start + height, X_start + width, Y_start + height, color);
	LCD_DrawBiggerLine(X_start + width, Y_start, X_start + width, Y_start + height, color);
